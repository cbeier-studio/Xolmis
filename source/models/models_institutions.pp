...
function TInstitution.Diff(const OldRec: TXolmisRecord; var Changes: TStrings): Boolean;
var
  aOld: TInstitution;
  R: String;
begin
  ...
  if not (OldRec is TInstitution) then
    Exit(False);

  aOld := TInstitution(OldRec);
...
procedure TInstitutionRepository.FindByRow(const ARow: TXRow; E: TXolmisRecord);
var
  Qry: TSQLQuery;
begin
  if not (E is TInstitution) then
    raise Exception.Create('FindByRow: Expected TInstitution');

  Qry := NewQuery;
  with Qry, SQL do
  try
    Clear;
    Add(xProvider.Institutions.SelectTable(swcNone));
