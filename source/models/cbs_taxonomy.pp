...
const
  ZooRanks: array[TZooRank] of String = ('D.', 'SD.', 'HK.', 'SK.', 'K.', 'sk.', 'ik.', 'pk.', 'SPh.', 'ph.',
...
uses
  cbs_locale, cbs_global, cbs_datatypes, cbs_data, cbs_datacolumns, cbs_getvalue, cbs_validations, cbs_users,
...
begin
  ...
  try
    ...
    //for i := Low(ZooRanks) to High(ZooRanks) do
...
begin
  ...
  ParentSp := ToSpecies;
  Ssp := TTaxon.Create(aSubspecies);
  aRankId := GetKey('taxon_ranks', 'rank_id', 'rank_acronym', ZooRanks[Ssp.RankId]);
...
begin
  ...
  try
    ...
    try
      ...
      ParamByName('rank_id').AsInteger := GetKey('taxon_ranks', 'rank_id', 'rank_acronym', ZooRanks[FRankId]);
      ...
      ParamByName('ioc_rank_id').AsInteger := GetKey('taxon_ranks', 'rank_id', 'rank_acronym', ZooRanks[FIocRankId]);
...
begin
  ...
  try
    ...
    JSONObject.Add('Taxon rank', ZooRanks[FRankId]);
    ...
    JSONObject.Add('Taxon rank (IOC)', ZooRanks[FIocRankId]);
