...
begin
  ...
  try
    ...
    Add('SELECT');
    Add('    pb.project_id,');
    Add('    SUM(pb.amount) - COALESCE(SUM(px.total_expense), 0) AS total_balance');
    Add('FROM project_budgets AS pb');
    Add('LEFT JOIN (');
    Add('  SELECT');
    Add('    project_id,');
    Add('    budget_id,');
    Add('    SUM(amount) AS total_expense');
    Add('  FROM project_expenses');
    Add('  GROUP BY project_id, budget_id');
    Add(') AS px ON pb.project_id = px.project_id AND pb.budget_id = px.budget_id');
    Add('WHERE (pb.project_id = :project_id)');
    Add('  AND (pb.active_status = 1)');
