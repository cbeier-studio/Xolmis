...
type
  ...
  TZooVernacularSQL = class(TInterfacedObject, IZooVernacularSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
  ...
  TZooSynonymsSQL = class(TInterfacedObject, IZooSynonymsSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
  ...
  TZooCountriesSQL = class(TInterfacedObject, IZooCountriesSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
...
begin
  Result :=
    'CREATE TABLE IF NOT EXISTS zoo_taxa (' +
      ...
      'distribution           TEXT,' +
      'accepted_status        BOOLEAN       DEFAULT (1),' +
      'user_inserted          INTEGER,' +
      'user_updated           INTEGER,' +
      'insert_date            DATETIME,' +
      'update_date            DATETIME,' +
...
var
  F: TTaxonFilter;
begin
  Result := 'SELECT t.taxon_id AS taxon_id, t.scientific_name AS scientific_name, t.formatted_name AS formatted_name ' +
    'FROM zoo_taxa AS t ';
  ...
  case aWhere of
    ...
    begin
      Result := Result +
        'WHERE (lower(t.scientific_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM)) ';
      ...
      Result := Result + 'AND (t.active_status = 1) ';
    end;
    swcActiveAll:
      Result := Result + 'WHERE (t.active_status = 1) ';
    swcActiveMarked:
      Result := Result + 'WHERE (t.marked_status = 1) AND (t.active_status = 1) ';
    swcInactive:
      Result := Result + 'WHERE (t.active_status = 0) ';
  end;
  ...
  begin
    ...
    Result := Result + 'SELECT s.taxon_id AS taxon_id, s.scientific_name AS scientific_name, s.formatted_name AS formatted_name ';
    Result := Result + 'FROM zoo_synonyms AS s ';

    case aWhere of
      swcNone: ;
      swcFindText:
        begin
          Result := Result + 'WHERE (lower(s.scientific_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM)) ';
          Result := Result + 'AND (s.active_status = 1) ';
        end;
      swcActiveAll:
        Result := Result + 'WHERE (s.active_status = 1) ';
      swcActiveMarked:
        Result := Result + 'WHERE (s.marked_status = 1) AND (s.active_status = 1) ';
      swcInactive:
...
constructor TZooVernacularSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;

function TZooVernacularSQL.CreateTable: String;
begin
  Result :=
    'CREATE TABLE IF NOT EXISTS zoo_vernacular (' +
...
function TZooVernacularSQL.Delete: String;
begin
  Result :=
    'DELETE FROM zoo_vernacular ' +
    'WHERE vernacular_id = :aid';
end;

function TZooVernacularSQL.Find(aWhere: TSQLWhereClause; aCriteria: TCriteriaType): String;
begin
  Result := 'SELECT vernacular_id, vernacular_name FROM zoo_vernacular ';
  ...
  case aWhere of
    ...
    begin
      Result := Result +
        'WHERE (lower(vernacular_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM)) ' +
          'AND (active_status = 1) ';
    end;
    swcActiveAll:
      Result := Result + 'WHERE (active_status = 1) ';
...
function TZooVernacularSQL.Insert: String;
begin
  Result :=
    'INSERT INTO zoo_vernacular (' +
      'vernacular_id, ' +
      'vernacular_name, ' +
      'taxon_id, ' +
      'language_id, ' +
...
function TZooVernacularSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result := 'SELECT * FROM zoo_vernacular ';

  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (vernacular_id = :cod) ';
    swcUpdateId:
      Result := Result + 'WHERE (vernacular_id = :vernacular_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (vernacular_id = -1) AND (active_status = 1) ';
    swcActiveAll:
      Result := Result + 'WHERE (active_status = 1) ';
    swcActiveMarked:
      Result := Result + 'WHERE (active_status = 1) AND (marked_status = 1) ';
...
function TZooVernacularSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT ' +
      'vernacular_id, ' +
...
function TZooVernacularSQL.Update: String;
begin
  Result :=
    'UPDATE zoo_vernacular SET ' +
      ...
      'user_updated = :user_updated, ' +
      'update_date = datetime(''now'',''subsec'') ' +
    'WHERE (vernacular_id = :vernacular_id) ';
end;

{ TZooSynonymsSQL }

constructor TZooSynonymsSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;

function TZooSynonymsSQL.CreateTable: String;
begin
  Result :=
    'CREATE TABLE IF NOT EXISTS zoo_synonyms (' +
      ...
      'authorship      VARCHAR (150),' +
      'formatted_name  VARCHAR (250),' +
      'valid_status    BOOLEAN       DEFAULT (0),' +
      'user_inserted   INTEGER,' +
      'user_updated    INTEGER,' +
      'insert_date     DATETIME,' +
      'update_date     DATETIME,' +
...
function TZooSynonymsSQL.Delete: String;
begin
  Result :=
    'DELETE FROM zoo_synonyms ' +
    'WHERE synonym_id = :aid';
end;

function TZooSynonymsSQL.Find(aWhere: TSQLWhereClause; aCriteria: TCriteriaType): String;
begin
  Result := 'SELECT synonym_id, scientific_name FROM zoo_synonyms ';
...
function TZooSynonymsSQL.Insert: String;
begin
  Result :=
    'INSERT INTO zoo_synonyms (' +
      'synonym_id, ' +
      'scientific_name, ' +
      'taxon_id, ' +
      'authorship, ' +
...
function TZooSynonymsSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result := 'SELECT * FROM zoo_synonyms ';

  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (synonym_id = :cod) ';
    swcUpdateId:
      Result := Result + 'WHERE (synonym_id = :synonym_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (synonym_id = -1) AND (active_status = 1) ';
    swcActiveAll:
      Result := Result + 'WHERE (active_status = 1) ';
    swcActiveMarked:
      Result := Result + 'WHERE (active_status = 1) AND (marked_status = 1) ';
...
function TZooSynonymsSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT ' +
      'synonym_id, ' +
...
function TZooSynonymsSQL.Update: String;
begin
  Result :=
    'UPDATE zoo_synonyms SET ' +
      'scientific_name = :scientific_name, ' +
      'taxon_id = :taxon_id, ' +
      'authorship = :authorship, ' +
      'formatted_name = :formatted_name, ' +
...
constructor TZooCountriesSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;

function TZooCountriesSQL.CreateTable: String;
begin
  Result :=
    'CREATE TABLE IF NOT EXISTS zoo_countries (' +
      'taxon_country_id INTEGER      PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL,' +
      'taxon_id        INTEGER       REFERENCES zoo_taxa (taxon_id) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL,' +
      'country_id      INTEGER       REFERENCES countries (country_id) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL,' +
      'user_inserted   INTEGER,' +
      'user_updated    INTEGER,' +
      'insert_date     DATETIME,' +
      'update_date     DATETIME,' +
...
function TZooCountriesSQL.Delete: String;
begin
  Result :=
    'DELETE FROM zoo_countries ' +
    'WHERE taxon_country_id = :aid';
end;

function TZooCountriesSQL.Find(aWhere: TSQLWhereClause; aCriteria: TCriteriaType; aTaxonId: Integer): String;
begin
  Result := 'SELECT zc.taxon_country_id, co.country_name AS country_name FROM zoo_countries AS zc ' +
    'LEFT JOIN countries AS co ON zc.country_id = co.country_id ';
    ...
    begin
      Result := Result +
        'WHERE (lower(country_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM)) ' +
          'AND (zc.active_status = 1) AND (zc.taxon_id = ' + IntToStr(aTaxonId) + ') ';
    end;
    swcActiveAll:
      Result := Result + 'WHERE (zc.active_status = 1) AND (zc.taxon_id = ' + IntToStr(aTaxonId) + ') ';
    swcActiveMarked:
      Result := Result + 'WHERE (zc.marked_status = 1) AND (zc.active_status = 1) AND (zc.taxon_id = ' + IntToStr(aTaxonId) + ') ';
    swcInactive:
      Result := Result + 'WHERE (zc.active_status = 0) AND (zc.taxon_id = ' + IntToStr(aTaxonId) + ') ';
  end;
end;

function TZooCountriesSQL.Insert: String;
begin
  Result :=
    'INSERT INTO zoo_countries (' +
      'taxon_country_id, ' +
      'taxon_id, ' +
      'country_id, ' +
      'user_inserted, ' +
...
function TZooCountriesSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT zc.*, ' +
      'co.country_name AS country_name ' +
    'FROM zoo_countries AS zc ' +
    'LEFT JOIN countries AS co ON zc.country_id = co.country_id ';

  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (zc.taxon_country_id = :cod) ';
    swcUpdateId:
      Result := Result + 'WHERE (zc.taxon_country_id = :taxon_country_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (zc.taxon_country_id = -1) AND (zc.active_status = 1) ';
    swcActiveAll:
      Result := Result + 'WHERE (zc.active_status = 1) ';
    swcActiveMarked:
      Result := Result + 'WHERE (zc.active_status = 1) AND (zc.marked_status = 1) ';
    swcInactive:
      Result := Result + 'WHERE (zc.active_status = 0) ';
    swcActiveParent: ;
    swcFindText: ;
  end;
end;

function TZooCountriesSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT ' +
      'taxon_country_id, ' +
  ...
  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (taxon_country_id = :cod) ';
    swcUpdateId:
      Result := Result + 'WHERE (taxon_country_id = :taxon_country_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (taxon_country_id = -1) AND (active_status = 1) ';
    swcActiveAll:
      Result := Result + 'WHERE (active_status = 1) ';
    swcActiveMarked:
      Result := Result + 'WHERE (active_status = 1) AND (marked_status = 1) ';
...
function TZooCountriesSQL.Update: String;
begin
  Result :=
    'UPDATE zoo_countries SET ' +
      'taxon_id = :taxon_id, ' +
      ...
      'user_updated = :user_updated, ' +
      'update_date = datetime(''now'',''subsec'') ' +
    'WHERE (taxon_country_id = :taxon_country_id) ';
end;
