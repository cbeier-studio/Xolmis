...
type
  ...
  TRolesSQL = class(TInterfacedObject, IRolesSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
  ...
  TPermissionsSQL = class(TInterfacedObject, IPermissionsSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
  ...
  TRolePermissionsSQL = class(TInterfacedObject, IRolePermissionsSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
  ...
  TImportProfilesSQL = class(TInterfacedObject, IImportProfilesSQL)
  private
    FBackend: TDatabaseBackend;
  public
    constructor Create(ABackend: TDatabaseBackend);
    ...
    begin
      Result := Result +
        'WHERE ((lower(full_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM)) ' +
            'OR (lower(user_name) ' + CRITERIA_OPERATORS[aCriteria] + ' lower(:VALPARAM))) ' +
          'AND (active_status = 1) ';
    end;
    swcActiveAll:
      Result := Result + 'WHERE (active_status = 1) ';
...
constructor TRolesSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;
    ...
    dbMaria:
      Result :=
        'CREATE TABLE IF NOT EXISTS roles (' +
          'role_id     INTEGER      PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, ' +
...
function TRolesSQL.Delete: String;
begin
  Result := 'DELETE FROM roles WHERE role_id = :aid';
end;

function TRolesSQL.Insert: String;
begin
  Result :=
    'INSERT INTO roles (role_name, description, can_delete) ' +
    'VALUES (:role_name, :description, :can_delete)';
end;

function TRolesSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  ...
  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (role_id = :aid) ';
    swcUpdateId:
    ...
    swcActiveAll, swcActiveMarked, swcInactive, swcActiveParent, swcFindText: ;
  end;
end;

function TRolesSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT role_id, role_name, description, can_delete ' +
    'FROM roles ';
...
function TRolesSQL.Update: String;
begin
  Result :=
    'UPDATE roles SET ' +
      'role_name = :role_name, ' +
...
constructor TPermissionsSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;
    ...
    dbMaria:
      Result :=
        'CREATE TABLE IF NOT EXISTS permissions (' +
          'permission_id     INTEGER      PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, ' +
          'permission_name   VARCHAR (80) NOT NULL UNIQUE, ' +
          'description       VARCHAR (255), ' +
          'permission_group  VARCHAR (80)' +
        ');';
    dbFirebird:
      Result :=
...
function TPermissionsSQL.Delete: String;
begin
  Result := 'DELETE FROM permissions WHERE permission_id = :aid';
end;

function TPermissionsSQL.Insert: String;
begin
  Result :=
    'INSERT INTO permissions (permission_name, description, permission_group) ' +
    'VALUES (:permission_name, :description, :permission_group)';
end;

function TPermissionsSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result := 'SELECT * FROM permissions ';

  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (permission_id = :aid) ';
    swcUpdateId:
      Result := Result + 'WHERE (permission_id = :permission_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (permission_id = -1) ';
    swcActiveAll, swcActiveMarked, swcInactive, swcActiveParent, swcFindText: ;
  end;
end;

function TPermissionsSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT permission_id, permission_name, description, permission_group ' +
    'FROM permissions ';
...
function TPermissionsSQL.Update: String;
begin
  Result :=
...
constructor TRolePermissionsSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;
...
function TRolePermissionsSQL.Delete: String;
begin
  Result := 'DELETE FROM role_permissions WHERE role_id = :role_id AND permission_id = :permission_id';
end;

function TRolePermissionsSQL.Insert: String;
begin
  Result := 'INSERT INTO role_permissions (role_id, permission_id) VALUES (:role_id, :permission_id)';
end;

function TRolePermissionsSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result := 'SELECT * FROM role_permissions ';
...
function TRolePermissionsSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result := 'SELECT role_id, permission_id FROM role_permissions ';
...
function TRolePermissionsSQL.Update: String;
begin
  Result :=
    'UPDATE role_permissions SET ' +
      'role_id = :role_id, ' +
      'permission_id = :permission_id ' +
    'WHERE (role_id = :old_role_id) AND (permission_id = :old_permission_id) ';
end;
...
constructor TImportProfilesSQL.Create(ABackend: TDatabaseBackend);
begin
  inherited Create;
  FBackend := ABackend;
end;
    ...
    dbMaria:
      Result :=
        'CREATE TABLE IF NOT EXISTS import_profiles (' +
          'profile_id    INTEGER       PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL,' +
...
function TImportProfilesSQL.Delete: String;
begin
  Result :=
...
function TImportProfilesSQL.Insert: String;
begin
  Result :=
...
function TImportProfilesSQL.SelectAll(aWhere: TSQLWhereClause): String;
begin
  Result :=
  ...
  case aWhere of
    swcNone: ;
    swcId:
      Result := Result + 'WHERE (profile_id = :aid) ';
    swcUpdateId:
      Result := Result + 'WHERE (profile_id = :profile_id) ';
    swcFieldValue:
      Result := Result + 'WHERE (%afield = :avalue) ';
    swcActiveEmpty:
      Result := Result + 'WHERE (profile_id ISNULL) ';
    swcActiveAll: ;
    swcActiveMarked: ;
    swcInactive: ;
    swcActiveParent: ;
...
function TImportProfilesSQL.SelectTable(aWhere: TSQLWhereClause): String;
begin
  Result :=
    'SELECT ' +
      'profile_id, ' +
...
function TImportProfilesSQL.Update: String;
begin
  Result :=
